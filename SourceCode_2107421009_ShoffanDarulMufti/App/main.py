...
def api_anggota():
    ...
    for column in output_db:
        ...
        anggota.append(content)
        content = {}
    return jsonify(anggota)

@application.route('/api/v1/buku')
def api_buku():
    output_db = getMethod("SELECT * FROM `buku`")
    buku = []
    for column in output_db:
        content = {'id_buku':column[0], 'judul':column[1], 'id_kode':column[2], 'penulis':column[3], 'penerbit':column[4], 'tahun':column[5], 'stok':column[6]}
        buku.append(content)
        content = {}
    return jsonify(buku)

@application.route('/api/v1/kode-buku')
def api_kode_buku():
    output_db = getMethod("SELECT * FROM `kode_buku`")
    kode_buku = []
    for column in output_db:
        content = {'id_kode':column[0], 'nama':column[1], 'deskripsi':column[2]}
        kode_buku.append(content)
        content = {}
...
def api_peminjaman():
    ...
    for column in output_db:
        ...
        peminjaman.append(content)
        content = {}
    return jsonify(peminjaman)

@application.route('/api/v1/pengembalian')
def api_pengembalian():
    ...
    for column in output_db:
        ...
        pengembalian.append(content)
        content = {}
    return jsonify(pengembalian)

@application.route('/api/v1/petugas')
def api_petugas():
    ...
    for column in output_db:
        ...
        petugas.append(content)
        content = {}
    return jsonify(petugas)

@application.route('/api/v1/rak')
def api_rak():
...
@application.route('/api/v1/relasi-rak-buku')
def api_relasi_rak_buku():
    ...
    for column in output_db:
        ...
        relasi_rak_buku.append(content)
        content = {}
    return jsonify(relasi_rak_buku)

@application.route('/api/v1/userlist')
def api_userlist():
